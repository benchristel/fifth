  ...
  module Instruction
    describe Add do
      it "adds two numbers without erroring" do
        ...
        expect(Add.invoke(vm)).to eq [VM.build(stack: [8]), nil]
      end

      it "errors given a single number" do
        vm = VM.build stack: [3]
        expect(Add.invoke(vm)).to eq [VM.build(stack: [3]), "`add` requires two operands"]
      end
    end
  end
end
